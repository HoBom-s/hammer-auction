...
/// <summary>
/// Background service that consumes messages from Kafka topics in batches.
/// </summary>
...
internal sealed partial class KafkaConsumerWorker : BackgroundService
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="KafkaConsumerWorker"/> class.
    /// </summary>
...
#pragma warning restore CA1031
    ...
    private async Task ProcessBatchAsync(List<ConsumeResult<string, string>> batch, CancellationToken ct)
    {
        ...
        AuctionDbContext dbContext = scope.ServiceProvider.GetRequiredService<AuctionDbContext>();

        foreach ((var topic, List<string> messages) in grouped)
            await _handlers[topic].HandleAsync(messages, dbContext, ct);
